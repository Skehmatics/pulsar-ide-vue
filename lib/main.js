...
class VueLanguageClient extends AutoLanguageClient {
  ...
  postInitialization(server) {
    // MAGIC: Volar, for reasons unknown to me, treats a space as a trigger character.
    //        This would be fine in the template portion, but causes problems in
    //        the script portion.
    const bad = [' ', '.'];
    const completeCap = server.capabilities.completionProvider;
    if (completeCap && Array.isArray(completeCap.triggerCharacters)) {
      completeCap.triggerCharacters = completeCap.triggerCharacters.filter((char) => !bad.includes(char));
    }
  }
  ...
  findTypescriptPath() {
    ...
    return path.join(ROOT, "node_modules", "typescript", "lib");
  }
